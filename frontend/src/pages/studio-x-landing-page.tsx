...
function TopNav({ solid }: { solid: boolean }) {
  ...
  return (
    <header className={`vex-topnav ${solid ? 'vex-topnav--solid' : ''}`}>
      <div className="vex-topnav__shell">
        <div className={`vex-topnav__bar ${solid ? '' : 'liquid-glass'}`}>
          <button
            type="button"
            className="vex-topnav__logo"
            onClick={() => go('hero')}
            aria-label="Home"
          >
            VEX
          </button>
          <nav className="vex-topnav__links" aria-label="Main">
          ...
          {authUi.loggedIn ? (
            <a
              className="vex-topnav__cta vex-topnav__cta--welcome"
              href="/account"
              title={authUi.displayName ?? 'Tài khoản của bạn'}
            >
              Welcome{authUi.displayName ? ` ${authUi.displayName}` : ''}
            </a>
          ) : (
            <a className="vex-topnav__cta" href="/auth">
              Đăng nhập / Đăng ký
            </a>
          )}
        </div>
